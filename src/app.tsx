...
import AudioSystem from "./typescript/audio";
...
//la variable de los eventos XD
export var game_events = new events.EventEmitter();
...
export class MainGame extends React.Component<props_main_game, state_main_game> {
    #game: Memory2 | null;
    ...
    async perdida(game_stats: game_stats) {
        ...
        this.setState(() => {
            ...
            return {
                ...
                back_menu_button:  <button onClick={() => {game_events.emit("to_menu", new_title)}}>Volver al menu</button>,
                info: {
                    score: game_stats.score, 
                    level: game_stats.level,
                }
...
////MAIN MENU 
var m_audio = new AudioSystem("menu");
interface props_main_menu {
    title: string;
};
interface state_main_menu {
    music: boolean;
    icon: ReactElement
};

export class MainMenuGame extends React.Component<props_main_menu, state_main_menu> {
    constructor(props: props_main_menu) {
        super(props);
        this.state = {
            music: true,
            icon:<BsVolumeUp />
        };
        this.update_audio = this.update_audio.bind(this);
    }

    #play_dif(dif: number) {
        game_events.emit("start_game", dif);
    }
    ...
    componentDidMount() {
        setTimeout(() => {m_audio.play(true);}, 500);
    }
    ...
    render(): React.ReactNode {
        ...
        <div id="main">
            ...
            <footer id="footer">
                <p id="build">(build 9)</p>
                <button id="volume_config" onClick={this.update_audio}>
                    {this.state.icon}
                </button>
            </footer>
        </div>);
